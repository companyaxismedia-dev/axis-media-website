    ...
    <section className="py-20 bg-gray-50" aria-labelledby="cms-heading">
      ...
      <div className="grid md:grid-cols-3 gap-10 max-w-7xl mx-auto px-6 mb-24">
        {services.map((s, i) => (
          <article
            key={i}
            className="group bg-white p-8 rounded-2xl shadow-lg border transition-all duration-300 hover:-translate-y-3 hover:shadow-2xl hover:bg-blue-600 hover:text-white"
          >
            <div className="transition-all duration-300 group-hover:scale-110 group-hover:text-white">
              {s.icon}
            </div>
            <h3 className="text-xl font-bold mt-4 mb-2 transition-colors duration-300 group-hover:text-white">
              {s.title}
            </h3>
      ...
      <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-12 items-center mb-24">
        <div>
          ...
          {processSteps.map((step, i) => (
            <div
              key={i}
              className="flex gap-3 mb-4 transition-all duration-300 hover:translate-x-2"
            >
              ...
              <div className="bg-white border border-t-0 rounded-b-xl shadow-md px-6 py-6">
                <div className="flex flex-wrap gap-4">
                  {section.items.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 px-4 py-2 border rounded-xl text-sm transition-all duration-300 hover:bg-purple-600 hover:text-white hover:-translate-y-1 hover:shadow-lg"
                    >
                      {item.icon}
                      <span>{item.name}</span>
                    </div>
